...
const LIST_SERVICE = ['chains','exchange', 'webhook'];
...
class Api {
   ...
   * @param {String} options.httpHeader          (optional) Set Params to http header
   * @memberof Api
   */
  constructor(options) {
    const defaultOpt = {
   ...
   * @memberof Api
   */
  getChainService(version = 'v1') {
    return this.getService('chains', version);
  }
   ...
   * @memberof Api
   */
  getExchangeService(version = 'v1') {
    return this.getService('exchange', version);
  }
   ...
   * @memberof Api
   */
  getWebhookService(version = 'v2') {
    return this.getService('webhook', version);
  }
  ...
  extendMethod(groupName, methods, ownner) {
    let group = {};
    if (ownner == null || ownner === undefined)
      ownner = this;
    ...
    methods.forEach(methodOpt => {
      ...
      method.setHttpHeader(this.options.httpHeader);
